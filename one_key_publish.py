...
# ==========================================
# Obsidian 的原始笔记目录
OBSIDIAN_VAULT = Path(r"D:\desktop\notes")
# Hugo 网站的 content 目录
...
ATTACHMENTS_DIRNAME = "__attachments__"
...
def sync_obsidian_to_hugo():
    ...
    if source_attachments.exists():
        ...
        print(
            f"  ℹ️ 提示: Obsidian 中未找到 '{ATTACHMENTS_DIRNAME}' 文件夹，跳过附件搬运。"
        )
    ...
    for source_file in source_files:
        try:
            ...
            raw_text = re.sub(
                r'^(date|lastmod|created|updated):\s*([^"\'\n\[\{]+)$',
                r'\1: "\2"',
                raw_text,
                flags=re.MULTILINE,
            )
                ...
                for key, value in post.metadata.items():
                    ...
                    if hugo_key in {"date", "lastmod"}:
                        ...
                        if not normalized_dt:
                            print(
                                f"  ⚠️ 跳过不可解析日期字段: {relative_path} -> {hugo_key}={value}"
                            )
                            continue
                    ...
                    if isinstance(value, str):
                        ...
                        list_str = ", ".join(
                            [f'"{str(item)}"' for item in value if item]
                        )
                        if list_str:
                        ...
                        new_frontmatter_parts.append(
                            f"{hugo_key} = {'true' if value else 'false'}"
                        )
                    else:
                        new_frontmatter_parts.append(f"{hugo_key} = {value}")
